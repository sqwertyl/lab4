...
schedule(void)
{
	...
	} else if (scheduling_algorithm == 1) { // strict priority
		while (1) {
			int i = 0;
			for (i = 0; i < NPROCS; i++) {
	...
	} else if (scheduling_algorithm == 2) { // self priority
		while (1) {
			pid_t current = pid;
			pid_t highest_priority = 0;
			while (1) {
				if (proc_array[current].p_state == P_RUNNABLE) {
					if (proc_array[current].p_priority < proc_array[highest_priority].p_priority || (proc_array[current].p_priority == proc_array[highest_priority].p_priority && current != pid)) {
			...
			if (proc_array[highest_priority].p_state == P_RUNNABLE) {
				run(&proc_array[highest_priority]);
			}
		}
	}
